...
public abstract class StealingSelectorThread extends Thread {
    ...
    public void register(IoTask task) {
        if ((task.ops & ~VALID_OPS) == 0) {
            throw new UnsupportedOperationException("Selector状态非法");
        }
     ...
     * 消费当前待注册的通道任务
     *
     * @param registerTaskQueue 待注册待通道
     */
     ...
     * 将单次就绪的任务缓存加入到总队列中
     */
    private void joinTaskQueue(final Queue<IoTask> readyTaskQueue,
     ...
     * 消费待完成的任务
     */
    private void consumeTodoTasks(final Queue<IoTask> readyTaskQueue,
        ConcurrentLinkedQueue<IoTask> registerTaskQueue) {
    ...
    public void run() {
        ...
        final List<IoTask> onceReadReadyTaskCache = new ArrayList<>(MAX_ONCE_READ_TASK);
        final List<IoTask> onceWriteReadyTaskCache = new ArrayList<>(MAX_ONCE_WRITE_TASK);
        ...
        try {
            while (isRunning) {
                    ...
                    if (selectionKey.isValid() && attachmentObj instanceof KeyAttachment) {
                        ...
                        try {
                            ...
                            if ((readyOps & SelectionKey.OP_READ) != 0 && onceReadTaskCount-- > 0) {
                                onceReadReadyTaskCache.add(attachment.task4Readable);
                                interestOps = interestOps & ~SelectionKey.OP_READ;
                            }

                            // 是否可写
                            if ((readyOps & SelectionKey.OP_WRITE) != 0 && onceWriteTaskCount-- > 0) {
                                onceWriteReadyTaskCache.add(attachment.task4Readable);
                            ...
                            // 当前连接被取消、断开时直接移除相关任务
                            if (attachment.task4Readable != null) {
                                onceReadReadyTaskCache.remove(attachment.task4Readable);
                                onceWriteReadyTaskCache.remove(attachment.task4Writable);
                            }
                        }
                    }
                    iterator.remove();
                ...
                // 判断本次是否有待执行的任务
                if (!onceReadReadyTaskCache.isEmpty()) {
                    // 加入到总队列中
                    joinTaskQueue(readyTaskQueue, onceReadReadyTaskCache);
                    onceReadReadyTaskCache.clear();
                }

                // 判断本次是否有待执行的任务
                if (!onceWriteReadyTaskCache.isEmpty()) {
                    // 加入到总队列中
                    joinTaskQueue(readyTaskQueue, onceWriteReadyTaskCache);
                    onceWriteReadyTaskCache.clear();
                }
